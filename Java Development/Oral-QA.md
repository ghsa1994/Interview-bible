...
## mysql基础和常识
### mysql怎么删除表效率更高
* Truncate table,可以删除表，和delete不同，在删除表的时候不会重构索引，而是删除所有索引，所以比较快。
* Drop table， 这个操作或删除表数据和表结构，删除后无法恢复。触发器和约束也会被删除。

## Oracle
### Oracle怎么分页
可以使用ROWNUM来分页，ROWNUM 是Oracle的行号，可以标识每一行的行号。
...
默认使用令牌桶策略算法，还有漏桶策略，计数器策略，IP地址限制等。令牌桶主要的是优点是能一定程度的突发流量，确定是无法应对短时间内大量请求。漏桶能精准的控制请求速度，但是处理请求时间不固定，间隔时间不太相等的服务时会有影响。计时器策略是指一秒通过多少请求，优点是简单，缺点长时间的大流量请求就不太适用了，IP限制的时主要防止恶意请求的。
### Nacos Gateway限流策略
...
###	说一下线程池的运行机制
线程池其实是一个hashset，如果在核心线程数量没有满则会执行任务，如果核心线程池满了则会加入到缓存队列，如果缓存队列也满了，则会检查是否超过了线程池的最大数量，如果没有超过也是执行任务，如果超过了则会使用hander处理拒绝加入队列。
### 线程池的拒绝策略有哪些
默认直接抛出异常，阻止系统的正常工作
调用者所在的线程来执行任务
丢弃队列中最老的一个任务和丢弃当前的任务。
...
##	怎么保证缓存和数据库数据的一致性
可以先更新数据库然后再更新缓存，如果更新缓存失败，那么等这个缓存过期后就可以重新获取数据库数据保持最终一致性。如果要保证强一致性，那么可以使用分布式事务，比如两段提交等。当然也可以使用消息队列要保持最终一致性，但是会增加复杂度。
##	Redis分布式锁使用要注意什么
保证业务代码加锁和解锁可以正常，要考虑粒度，注意锁超时，考虑是否持久化，考虑高可用，尽量设置的字段有一些特点避免误删。

# MangoDB
## 芒果db可以用来存储什么
可以存储非结构化数据，比如文档、图像、视频、音频等。因此，可以使用MangoDB存储对象，但是它不是专门用于存储对象的数据库
...
## 卡夫卡和rocketmq有什么区别
卡夫卡对批量数据处理比较好，因为卡夫卡是吧消息封装成一批一起发送的，rocketmq则是使用单条发送的。卡夫卡会直接持久化在磁盘中，rocketmq如果没有开启同步刷盘则会使用内存个磁盘混合存储来提高性能。卡夫卡架构是基于订阅模式的，RocketMQ默认是基于队列模式的。还有卡夫卡在11版本之前不支持死信队列。
...
# 云服务
## 云服务有哪些用来存储对象
比如说阿里云会有oss云对象存储服务，一般用来存储非结构化的数据，像是图片吗，音频，文本文件等。主要的作用是让应用程序和用户之间传输更快更可靠，同时减轻服务器的负担。
## 云服务都有哪些
有云服务器，关系型数据库，对象存储，文件存储，CDN，还有一些AI服务，物联网服务等
...
## Http1.1和1.0有什么不同
* Http1.1默认是长连接，1.0默认是短连接
* 1.1加了一些状态码。
* 1.1可以分割消息，可以实现流式传输和动态生成内容。
* 1.1必须包含host头信息
另外在缓存处理比较完善，支持多种传输编码范式和支持range头
##	数据结构
###	怎么实现一个链表
...
然后定义二叉树类，插入查找和遍历等。
## 分布式唯一ID的生成
可以使用UUID，雪花算法。UUID不是纯数字的，而且太长，会造成性能上不是太好，雪花算法是纯数字的，而且可以在分布式系统中生成，现在很多唯一ID生成都是基于雪花算法的。比如美团的leaf/阿里的flake还有百度的idgen
#	编码技巧
##	查找list集合中某个元素
